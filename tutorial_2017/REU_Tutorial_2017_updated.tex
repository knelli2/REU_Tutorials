...
\author{Abid Khan, Sean Connelly, Cunwei Fan, and Patchara Wongsutthikoson\\
	Edited: Eric Connelly, Imran Sultan}
...
\section{Linux}
	...
	\item[grep]	This command outputs items that match your input pattern strings.
			...
			when used with pipe (|).
			eg: \textbf{grep 1200 foo.txt} will output the lines that has "1200" in file foo.txt.
        ...
        \item[pipe (|)] This command makes compound commands available, allowing you to perform more complicated
			...
			the next one. e.g: \textbf{ls | grep foo}, this use the output of ls command as the input for grep
	...
	\item[redirect (>)] This command will redirect the output display to another
			...
			e.g \textbf{echo hello world > new.txt} will modify the file "new.txt" (or make a new file "new.txt", if it doesn't exist) to have "hello world" as its content.
			Note that ">" erases the file before writing, while ">{}>" appends to it.

        \item[sort] 	What do you think it does?
        
        \item[uniq]	Delete \textit{consecutive} duplicate lines of a file. See \textbf{awk} for how to delete \textit{non-consecutive} duplicate lines.

	\item[scp] 	At times we will be needing to send files or data to each other for collaboration. How do we do that? EMAIL?! Where's the fun in that? To send a file from one machine to another type \\
			\textbf{scp filename colten1@machine.physics.illinois.edu:/home/colten1/Desktop}, \\
			...
			\textbf{scp colten1@machine.physics.illinois.edu:/full/path/to/file /path/to/folder} \\
			If you want to transfer a directory instead, put an \textbf{-r} after scp
	...
	\item[scripting] Bash supports a few simple common functions like for loop
			...
			you defined earlier, put a dollar sign (\$) in the front. eg. \textbf{var=1; echo \$var}
	...
	\item[awk (optional)] 	Technically a programming language, but useful for editing text files. 
			...
			\textbf{awk '{print \$2}'} (print 2nd column of a file), and \textbf{awk '!a[\$0]++'} (remove non-consecutive duplicate lines) \\
       		   	Refer to http://www.pement.org/awk/awk1line.txt
       		   	
	\item[sed (optional)] 	Short for `stream editor'. 
...
\subsection{Exercises}
    Quick note: don't spend too much time trying to work these out yourselves if you get stuck; use Google to whenever you need to. These are mainly designed for future reference.
    \begin{enumerate}
	\item Use \textbf{wc -l} and \textbf{ls} to count the number of files in a folder.
	\item Use terminal to remove the extention part of the file name. eg. "foo.txt" -> "foo".
	\item Use the \textbf{head} and \textbf{tail} command to remove the first and last line of a text file.
	\item Use \textbf{grep} command to delete all the lines in a text file that contains a specific pattern (say "NaN"). \textit{Hint: \textbf{grep -v}}
	\item Use sort command (you can look it up on google), sort a text file based on the value in the second column in each line. So your text file has data a matrix form (multiple rows and multiple columns).
